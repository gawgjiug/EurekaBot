  ...
  if (
    ...
    const results = await Promise.all(
      poll.options.map(async (option, index) => {
        ...
        const count = reaction ? reaction.count - 1 : 0; // 봇의 반응을 제외
...
const port = process.env.PORT || 3000; // Replit에서 포트를 설정합니다

// 기본 라우트 설정
app.get('/', (req, res) => {
  res.send('봇이 작동 중입니다!'); // 웹 서버가 응답할 메시지
});

// 웹 서버를 지정된 포트에서 실행합니다
app.listen(port, () => {
